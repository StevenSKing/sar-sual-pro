...
import { getConfig, coinDel, startOrStop, addOrUpdate, sellAll, getPos, getOrders } from '@/http/api';
...
function handleStop(record, onSuccess) {
  Modal.confirm({
    title: '温馨提示',
    content: <p>确定{record.status === 1 ? '运行' : '停止'} {record.type === 'hy' ? '合约' : '现货'} {record.coin} 吗?</p>,
    onOk: () => {
      try {
...
function SettingCoin() {
  ...
  const columns_ordeer = [
    ...
    {
      dataIndex: 'origQty', title: '数量', width: 80
    },
    {
      dataIndex: 'status', title: '状态', width: 80
    },
    {
      dataIndex: 'side', title: '方向', width: 80
    },
    {
      dataIndex: 'positionSide', title: '持仓方向', width: 80
    },
    {
  ...
  return (
    < div style={{ height: '1000px' }}>
      ...
      <PostForm
        ...
        onUpdate={(values) => {
          console.log('values=onUpdate=>', values)
          setModalActionLoading(true)
          try {
            addOrUpdate(values).then(res => {
              if (res.data.code === 200) {
      ...
      >
        <Table
          ...
          // pagination={{ ...antdPaginationAdapter(orderData.pagination) }}
          // onChange={(pagination, filters, sorter) => {
